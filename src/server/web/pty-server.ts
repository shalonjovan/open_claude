...
const MAX_SESSIONS = Number.parseInt(process.env.MAX_SESSIONS ?? "10", 10);
const MAX_SESSIONS_PER_USER = Number.parseInt(
	process.env.MAX_SESSIONS_PER_USER ?? "3",
	10,
);
const MAX_SESSIONS_PER_HOUR = Number.parseInt(
	process.env.MAX_SESSIONS_PER_HOUR ?? "10",
	10,
);
const ALLOWED_ORIGINS = process.env.ALLOWED_ORIGINS?.split(",") ?? [];
const GRACE_PERIOD_MS = Number.parseInt(
	process.env.SESSION_GRACE_MS ?? String(5 * 60_000),
	10,
);
const SCROLLBACK_BYTES = Number.parseInt(
...
server.listen(PORT, HOST, () => {
	...
	console.log(
		`  Max sessions: ${MAX_SESSIONS} (${MAX_SESSIONS_PER_USER} per user)`,
	);
