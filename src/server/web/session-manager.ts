...
export class UserHourlyRateLimiter {
	...
	private recent(userId: string): number[] {
		...
		const filtered = (this.attempts.get(userId) ?? []).filter(
			(t) => t > cutoff,
		);
...
export class SessionManager {
		...
		if (this.isUserRateLimited(userId)) {
			ws.send(
				JSON.stringify({
					type: "error",
					message:
						"Too many sessions created recently. Please wait before starting a new session.",
				}),
			);
	...
	private wireWsEvents(token: string, ws: WebSocket, pty: IPty): void {
		...
		ws.on("error", (err) => {
			console.error(
				`[session ${token.slice(0, 8)}] WebSocket error:`,
				err.message,
			);
			handleClose();
		});
