...
function toChecksumAddress(address: string): string {
	...
	for (let i = 0; i < addr.length; i++) {
		checksummed +=
			Number.parseInt(hash[i], 16) >= 8 ? addr[i].toUpperCase() : addr[i];
	}
	return checksummed;
}
...
function deriveAddress(privateKeyHex: string): string {
	const { createPublicKey, createPrivateKey } =
		require("node:crypto") as typeof import("crypto");
...
export function getX402Config(): X402WalletConfig {
	...
	return (
		((config as Record<string, unknown>).x402 as X402WalletConfig) ?? {
			...X402_DEFAULTS,
		}
	);
}
...
export function getX402PrivateKey(): string | undefined {
	...
	const config = getGlobalConfig();
	return (config as Record<string, unknown>).x402PrivateKey as
		| string
		| undefined;
}
