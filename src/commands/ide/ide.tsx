...
function IDEScreen(t0) {
		...
		t2 = (value) => {
			if (value !== "None" && shouldShowAutoConnectDialog()) {
				...
				if (value === "None" && shouldShowDisableAutoConnectDialog()) {
					...
					onSelect(
						availableIDEs.find((ide) => ide.port === Number.parseInt(value)),
					);
				}
			}
		};
	...
	if ($[24] !== availableIDEs) {
		t7 = availableIDEs.length !== 0 && availableIDEs.some(_temp2) && (
			<Box marginTop={1}>
				<Text color="warning">
					Note: Only one Claude Code instance can be connected to VS Code at a
					time.
				</Text>
			</Box>
		);
	...
	if ($[2] !== availableIDEs || $[3] !== onSelectIDE) {
		t2 = (value) => {
			const selectedIDE = availableIDEs.find(
				(ide) => ide.port === Number.parseInt(value),
			);
